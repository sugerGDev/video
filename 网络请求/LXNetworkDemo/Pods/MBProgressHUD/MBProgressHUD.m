...
- (void)showAnimated:(BOOL)animated {
    ...
    if (self.graceTime > 0.0) {
        NSTimer *timer = [NSTimer timerWithTimeInterval:self.graceTime target:[YYWeakProxy proxyWithTarget:self] selector:@selector(handleGraceTimer:) userInfo:nil repeats:NO];
...
- (void)hideAnimated:(BOOL)animated {
    ...
    if (self.minShowTime > 0.0 && self.showStarted) {
        ...
        if (interv < self.minShowTime) {
            NSTimer *timer = [NSTimer timerWithTimeInterval:(self.minShowTime - interv) target:[YYWeakProxy proxyWithTarget:self] selector:@selector(handleMinShowTimer:) userInfo:nil repeats:NO];
            [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
...
- (void)hideAnimated:(BOOL)animated afterDelay:(NSTimeInterval)delay {
    ...
    NSTimer *timer = [NSTimer timerWithTimeInterval:delay target:[YYWeakProxy proxyWithTarget:self] selector:@selector(handleHideTimer:) userInfo:@(animated) repeats:NO];
    [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
...
- (void)setNSProgressDisplayLinkEnabled:(BOOL)enabled {
    ...
    if (enabled && self.progressObject) {
        ...
        if (!self.progressObjectDisplayLink) {
            self.progressObjectDisplayLink = [CADisplayLink displayLinkWithTarget:[YYWeakProxy proxyWithTarget:self] selector:@selector(updateProgressFromProgressObject)];
        }
    } else {
