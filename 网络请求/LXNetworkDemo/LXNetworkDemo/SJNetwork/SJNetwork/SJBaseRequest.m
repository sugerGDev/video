...
/** 请求id(app生命周期内递增) */
@property (nonatomic, strong) YYThreadSafeArray *requestIdList;
/** 缓存对象 */
...
- (YYThreadSafeArray *)requestIdList
{
    if (_requestIdList == nil) {
        _requestIdList = [[YYThreadSafeArray alloc] initWithCapacity:20];
    }
...
- (NSInteger)loadDataWithParams:(NSDictionary *)params headers:(NSDictionary*)headers uploads:(NSDictionary*) uploads
{
    if (!([self.child respondsToSelector:@selector(requestType)] &&
          [self.child respondsToSelector:@selector(requestUri)])) {
        NSAssert(NO, @"接口类需实现requestType 与 requestUri 方法");
        return 0;
    }
        ...
        if ([self isCorrectWithParamsData:apiParams]) {    //检查参数正确性
            ...
            if ([self isReachable]) {              // 有网络
                switch (self.child.requestType)    // get/post/upload
                {
                    ...
                    {
                        requestId = [[SJRequestProxy sharedInstance] callGETWithParams:apiParams
                                                                                   uri:self.child.requestUri
                                                                               headers:headers methodName:self.getMethodName
                                                                         isDefaultHost:self.child.isDefaultHost success:^(SJResponse *response) {
                                                                             @strongify(self);
                    ...
                    {
                        requestId = [[SJRequestProxy sharedInstance] callPOSTWithParams:apiParams
                                                                                    uri:self.child.requestUri
                                                                                headers:headers methodName:self.getMethodName
                                                                          isDefaultHost:self.child.isDefaultHost success:^(SJResponse *response) {
                                                                              @strongify(self);
                    ...
                    {
                        requestId = [[SJRequestProxy sharedInstance]callUPLOADWithParams:apiParams
                                                                                     url:self.child.requestUri
                                                                                 headers:headers uploads:uploads methodName:self.getMethodName
                                                                           isDefaultHost:(BOOL)self.child.isDefaultHost success:^(SJResponse *response) {
                                                                               @strongify(self);
                                                                               [self successedOnCallingAPI:response];
...
#warning SJBaseRequest 实际项目用真是的 token 作为缓存的 key
    //    NSString *methodName = [NSString stringWithFormat:@"%@_%@_%@",[self convertRequestType:self.child.requestType], SJAPPDelegate.token ? SJAPPDelegate.token : @"token", self.child.requestUri];
    NSString *methodName = [NSString stringWithFormat:@"%@_%@_%@",[self convertRequestType:self.child.requestType], @"token", self.child.requestUri];
