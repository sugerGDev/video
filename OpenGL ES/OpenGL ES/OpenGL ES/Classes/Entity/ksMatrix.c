...
{
	/**
	 * fovy 定义了 camera 在 y 方向上的视线角度（介于 0 ~ 180 之间），aspect 定义了近裁剪面的宽高比 aspect = w/h，而 nearZ 和 farZ 定义了从 Camera/Viewer 到远近两个裁剪面的距离（注意这两个距离都是正值）。这四个参数同样也定义了一个视锥体。
在 OpenGL ES 2.0 中，我们也需要自己实现该函数。我们可以通过三角公式 tan(fovy/2)  = (h / 2)/nearZ 计算出 h ，然后再根据 w =  h * aspect 计算出 w，这样就可以得到 left, right, top, bottom, zNear, zFar 六个参数，代入在介绍视锥体时提到的公式即可。
	 */
	float frustumW, frustumH;

	frustumH = tanf( fovy / 360.f * M_PI ) * nearZ;
